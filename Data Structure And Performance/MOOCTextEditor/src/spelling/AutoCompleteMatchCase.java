...
public class AutoCompleteMatchCase implements  Dictionary, AutoComplete {
     ...
     */@Override
     public List<String> predictCompletions(String prefix, int numCompletions) 
     {
    	 ...
    	 // 2. add the children node, build up the Q and the listOfCompletion
    	 int totalCompletion = 0;
    	 nodeQueue.add(curNode);
    	 while ((totalCompletion < numCompletions) &&
    			 (nodeQueue.size() > 0)){
    		 curNode = nodeQueue.remove();
    		 if (curNode.endsWord()) {
    			 listOfWord.add(curNode.getText());
    			 totalCompletion++;
    		 }
    		 for (Character ch : curNode.getValidNextCharacters()) {
    			 nodeQueue.add(curNode.getChild(ch));
    		 }
