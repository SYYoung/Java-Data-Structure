...
public class Point implements Comparable<Point> {
    ...
    public static void main(String[] args) {
        ...
        // 2. sort the points in natural way
        StdOut.println("Input points: ");
        for (Point eachPoint : points)
            StdOut.println(eachPoint);
        Arrays.sort(points);
        // 3. print out the sorted array
        StdOut.println("after natural sorting: ");
        for (Point eachPoint : points)
            StdOut.println(eachPoint);
        // 4. get the slopes from the first point in the array
        Point origin = points[0];
        for (Point eachPoint : points) {
            double slope = origin.slopeTo(eachPoint);
            StdOut.println("Slope between origin and " + eachPoint + " = " + slope);
        }
        // 5. sort the slop order
        Arrays.sort(points, origin.slopeOrder());
        StdOut.println("after sorting by slopOrder: ");
        for (Point eachPoint : points)
            StdOut.print(eachPoint);
    }
