...
public class MapGraph {
	...
	int numNode;
	int numEdge;
	HashMap<GeographicPoint, Double> curDistance;
	...
	public MapGraph()
	{
		...
		numNode = 0;
		numEdge = 0;
		curDistance = new HashMap<GeographicPoint, Double>();
	}
	...
	*/
	...
	private double calculateStartCost(double curDistance, MapEdge edge)
	{
		double startCost = 0;
	...
	private double calculatePredictedCost(double curDistance, MapEdge edge,
			MapNode endNode, GeographicPoint goal, int searchType)
	{
		double predictCost = 0;
		double startCost = 0;
		startCost = curDistance + edge.getDistance();
		if (searchType == DIJKSTRA)
		{
			predictCost = startCost;
		}
		else if (searchType == ASTAR)
		{
			predictCost = startCost + endNode.getLoc().distance(goal);
		}
		return predictCost;
	}
		...
		while (queue.size() > 0) {
			...
			if (!visitedLoc.contains(curNode.getLoc()))
			{
				...
				for (MapEdge edge : curNode.getOutEdge()) {
					...
					if (!visitedLoc.contains(endPoint)) {
						MapNode endNode = nodeMap.get(endPoint);
						double startCost = calculateStartCost(curDistSoFar, edge);
						double predictCost = calculatePredictedCost(curDistSoFar, edge, endNode,
													goal, searchType);
						if (endNode.getDistSoFar() > startCost)
						{
							endNode.setDistSoFar(startCost);
							endNode.setPredictedDist(predictCost);
							queue.add(nodeMap.get(endPoint));
							parentMap.put(endPoint, curNode.getLoc());
							nodeSearched.accept(endPoint);
						}
...
class MapNode implements Comparable<MapNode> {
	...
	*/
	...
	{
		if (this.predictedDist > node2.predictedDist)
			return 1;
		else if (this.predictedDist < node2.predictedDist)
			return -1;
		else
			return 0;
	}
	...
	public void setPredictedDist(double val)
	{
		predictedDist = val;
	}
