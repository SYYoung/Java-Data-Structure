...
public class BoggleSolver {
    ...
    private Bag<Integer>[] neighborList;
    ...
    public Iterable<String> getAllValidWords(BoggleBoard board) {
        ...
        for (int i = 0; i < rows * cols; i++)
            neighborList[i] = new Bag<Integer>();
        // neighborList = new ArrayList<Integer>[rows * cols] ();
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                buildAdj(i, j, rows, cols, neighborList[i * cols + j]);
            }
        }
    ...
    private Bag<Integer> getAdj(int row, int col, int totalRow, int totalCol) {
        return neighborList[row * totalCol + col];
    }

    private void buildAdj(int row, int col, int totalRow, int totalCol, Bag<Integer> neigh) {
        for (int i = -1; i <= 1; i++)
            for (int j = -1; j <= 1; j++) {
                if (i == 0 && j == 0) continue;
                int r = row + i;
